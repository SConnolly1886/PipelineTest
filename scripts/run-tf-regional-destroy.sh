...
usage_help="Usage: $0 [-t <env>_<region>/<team>/<version>] [-b <tf_backend_config_prefix>] [-g <global_resource_deploy_from_region>] [-r <tf_state_region>]"
usage() { echo "$usage_help" 1>&2; exit 1; }

# Default values if none provided
tag="dev_us-east-1/research/1.0"
global_resource_deploy_from_region="us-east-1"
tf_state_region="us-east-1"
while getopts ":t:r:b:g:" o; do
    case "${o}" in
        t)
            tag=${OPTARG}
            ;;
...
if [ $OPTIND -eq 1 ]; then echo "$usage_help"; echo "No arguments were passed...Setting to default values:"; fi
...
echo "terraform plan -var-file "environments/${ENV}/${TEAM}/variables.tfvars" ${REGION_TFVARS} -var "env=${ENV}" -var "region=${REGION}" -var "team=${TEAM}" -target ${TARGET_MODULE} -out=tfplan"
terraform plan -destroy -var-file "environments/${ENV}/${TEAM}/variables.tfvars" ${REGION_TFVARS} -var "env=${ENV}" -var "region=${REGION}" -var "team=${TEAM}" -target ${TARGET_MODULE} -out=tfplan -compact-warnings